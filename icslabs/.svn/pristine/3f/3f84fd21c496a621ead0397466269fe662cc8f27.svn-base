...
stat_t nexti(y64sim_t *sim)
{
    ...
    flag = get_byte_val(sim->m,next_pc,&reg_Get);
    if (icode == I_RRMOVQ || icode == I_RMMOVQ || icode == I_MRMOVQ || icode == I_ALU) {
        ...
        rA = GET_REGA(reg_Get);
        rB = GET_REGB(reg_Get);
        if (rA >= REG_NONE || rA < REG_RAX || rB >= REG_NONE || rB < REG_RAX) {
            err_print("PC = 0x%lx, Invalid instruction %.2x", sim->pc, codefun);
            return STAT_INS;
        }
        next_pc++;
    ...
    if (icode == I_IRMOVQ) {
        ...
        rA = GET_REGA(reg_Get);
        rB = GET_REGB(reg_Get);
        if (rA != REG_NONE || rB >= REG_NONE || rB < REG_RAX) {
            err_print("PC = 0x%lx, Invalid instruction %.2x", sim->pc, codefun);
            return STAT_INS;
        }
        next_pc++;
    ...
    if (icode == I_PUSHQ || icode == I_POPQ) {
        ...
        rA = GET_REGA(reg_Get);
        rB = GET_REGB(reg_Get);
        if (rA >= REG_NONE || rA < REG_RAX || rB != REG_NONE) {
            err_print("PC = 0x%lx, Invalid instruction %.2x", sim->pc, codefun);
            return STAT_INS;
        }
        next_pc++;
    ...
    /* get immediate i needed (8 bytes) */
    long_t imm;

    flag = get_long_val(sim->m,next_pc,&imm);
    ...
    switch (icode) {
      ...
      case I_RRMOVQ:  /* 2:x regA:regB */
        ...
        temp = get_reg_val(sim->r,rA);
        if ((int)ifun == C_YES)
            set_reg_val(sim->r,rB,temp);
        if ((int)ifun == C_LE && (((sf ^ of) | zf) & 1))
            set_reg_val(sim->r,rB,temp);
        if ((int)ifun == C_L && ((sf ^ of) & 1))
            set_reg_val(sim->r,rB,temp);
        if ((int)ifun == C_E && (zf & 1))
            set_reg_val(sim->r,rB,temp);
        if ((int)ifun == C_NE && (~zf & 1))
            set_reg_val(sim->r,rB,temp);
        if ((int)ifun == C_GE && (~(sf ^ of) & 1))
            set_reg_val(sim->r,rB,temp);
        if ((int)ifun == C_G && ((~(sf ^ of) & ~zf) & 1))
            set_reg_val(sim->r,rB,temp);
        sim->pc = next_pc;
        break;
      ...
      case I_RMMOVQ: /* 4:0 regA:regB imm */
        ...
        temp = get_reg_val(sim->r,rA);
        temp_2 = get_reg_val(sim->r,rB);
        set_long_val(sim->m,temp_2 + imm,temp);
        sim->pc = next_pc;
        break;
      ...
      case I_MRMOVQ: /* 5:0 regB:regA imm */
        ...
        temp = get_reg_val(sim->r,rB);
        flag = get_long_val(sim->m,temp + imm,&temp_2);
        if(flag == FALSE) {
            err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc,temp + imm);
            return STAT_ADR;            
        } 
        set_reg_val(sim->r,rA,temp_2);
        sim->pc = next_pc;
        break;
      ...
      case I_ALU: /* 6:x regA:regB */
        ...
        temp_2 = get_reg_val(sim->r,rB);
        if ((int)ifun == A_ADD) {
            set_reg_val(sim->r,rB,temp + temp_2);
            sim->cc = compute_cc(A_ADD,temp,temp_2,temp  + temp_2);
        }
        if ((int)ifun == A_SUB) {
            set_reg_val(sim->r,rB,temp_2 - temp);
            sim->cc = compute_cc(A_SUB,temp,temp_2,temp_2 - temp);
        }
        if ((int)ifun == A_AND) {
            set_reg_val(sim->r,rB,temp & temp_2);
            sim->cc = compute_cc(A_AND,temp,temp_2,temp & temp_2);
        }
        if ((int)ifun == A_XOR) {
            set_reg_val(sim->r,rB,temp ^ temp_2);
            sim->cc = compute_cc(A_XOR,temp,temp_2,temp ^ temp_2);
        }
        sim->pc = next_pc;
        break;
      ...
      case I_CALL: /* 8:x imm */
        ...
        temp = get_reg_val(sim->r,REG_RSP);
        temp = temp - 8;
        set_reg_val(sim->r,REG_RSP,temp);
        if(!set_long_val(sim->m,temp,next_pc)) {
            err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, temp);
            return STAT_ADR;
        }
        sim->pc = imm;
        break;
      ...
      case I_RET: /* 9:0 */
        ...
        temp = get_reg_val(sim->r,REG_RSP);
        if (!get_long_val(sim->m,temp,&temp_2)) {
            err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, temp);
            return STAT_ADR; 
        }
        sim->pc = temp_2;
        set_reg_val(sim->r,REG_RSP,temp + 8);
        break;
      ...
      case I_PUSHQ: /* A:0 regA:F */
        ...
        temp = get_reg_val(sim->r,REG_RSP);
        temp_2 = get_reg_val(sim->r,rA);
        temp = temp - 8;
        set_reg_val(sim->r,REG_RSP,temp);
        if (!set_long_val(sim->m,temp,temp_2)) {
            err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, temp);
            return STAT_ADR; 
        }
        sim->pc = next_pc;
